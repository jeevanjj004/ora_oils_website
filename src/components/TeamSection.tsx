...
function TeamCard({ member, index }: { member: typeof MEMBERS[0]; index: number }) {
  ...
  return (
    <motion.div
      ...
      className="group"
    >
      <div className="bg-gradient-to-br from-[#0A1A12] to-[#06110B] rounded-xl sm:rounded-2xl overflow-hidden border border-[var(--color-soft-gold)]/15 transition-all duration-400 group-hover:border-[var(--color-soft-gold)]/35 h-full flex flex-col justify-between relative">

        <div>
          {/* Photo area */}
          <div className="pt-5 sm:pt-6 md:pt-8">
            <div className="w-20 h-20 sm:w-24 sm:h-24 md:w-28 md:h-28 lg:w-32 lg:h-32 mx-auto">
              <div className="relative w-full h-full rounded-full overflow-hidden bg-gradient-to-br from-[#1a2e22] to-[#0a1a10] ring-1 ring-[var(--color-soft-gold)]/20 group-hover:ring-[var(--color-soft-gold)]/40 transition-all duration-400">
                {!imageLoaded && !imageError && (
                  <div className="absolute inset-0 flex items-center justify-center bg-[#0a1a10]">
                    <div className="w-4 h-4 sm:w-5 sm:h-5 rounded-full border-2 border-[var(--color-soft-gold)]/30 border-t-[var(--color-soft-gold)] animate-spin" />
                  </div>
                )}
                <img
                  src={imgSrc}
          ...
          <div className="text-center mt-3 sm:mt-4 px-2">
            <h3 className="text-base sm:text-lg md:text-xl font-serif font-light text-white">
              {member.name}
            </h3>
            <p className="text-[8px] sm:text-[9px] md:text-[10px] uppercase tracking-[0.3em] text-[var(--color-soft-gold)]/60 font-medium mt-0.5">
              {member.role}
            </p>
          </div>
        </div>

        {/* Contact Info - Centered with icons on left */}
        <div className="mt-4 pb-5 sm:pb-6">
          <div className="flex flex-col items-center gap-1.5">
            {/* Email */}
            <div className="flex items-center justify-center gap-1.5">
              <EnvelopeSimple 
                size={10} 
                className="text-white/40 group-hover:text-[var(--color-soft-gold)] transition-colors duration-300" 
                weight="fill"
              />
              <a
                href={`mailto:${member.email}`}
                className="text-[9px] sm:text-[10px] text-white/40 hover:text-[var(--color-soft-gold)] transition-colors duration-300 text-center"
              >
                {member.email}
              </a>
            </div>
      ...
      <div className="max-w-4xl mx-auto relative z-10">
        ...
        <div ref={headerRef} className="text-center mb-8 sm:mb-10 md:mb-12 lg:mb-16">
          ...
          <motion.div
            ...
            className="h-px bg-[var(--color-soft-gold)]/40 mx-auto mt-3"
          />
        </div>
